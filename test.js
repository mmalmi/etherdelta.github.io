...
describe('Test', function test() {
  ...
  describe('Contract scenario', () => {
    ...
    // MARGIN
    it('Should do some margin accounts initiated onchain', (done) => {
    ...
    it('Should place a margin account order onchain, check availableVolume and amountFilled, then cancel', (done) => {
      function testCancel(expiresIn, orderNonce, tokenGet, tokenGive, amountGet, amountGive, amount, account1, callback) {
        let expires = expiresIn;
        web3.eth.getBlockNumber((err, blockNumber) => {
          if (err) callback(err);
          expires += blockNumber;
          utility.testSend(web3, contractEtherDelta, contractEtherDeltaAddr, 'marginOrder', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, { gas: 1000000, value: 0 }], account1, undefined, 0, (err2) => {
            assert.equal(err2, undefined);
            utility.testCall(web3, contractEtherDelta, contractEtherDeltaAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0'], (err3, result3) => {
              assert.equal(result3.equals(amountGet), true);
              utility.testCall(web3, contractEtherDelta, contractEtherDeltaAddr, 'amountFilled', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0'], (err4, result4) => {
    ...
    it('Should open a margin account and check available volume depletion', (done) => {
      web3.eth.getBlockNumber((err, blockNumber) => {
        ...
        const amount = amountGet.div(new BigNumber(2));
        utility.sign(web3, user, hash, undefined, (err2, sig) => {
          utility.testSend(web3, contractEtherDelta, contractEtherDeltaAddr, 'openMarginAccount', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s, amount, { gas: 1000000, value: 0 }], accounts[2], undefined, 0, (err3) => {
            assert.equal(err3, undefined);
            utility.testCall(web3, contractEtherDelta, contractEtherDeltaAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s], (err4, result4) => {
              assert.equal(result4.equals(amountGet.minus(amount)), true);
              done();
    ...
    it('Should open a self margin account and check available volume depletion', (done) => {
      web3.eth.getBlockNumber((err, blockNumber) => {
        ...
        const amount = amountGet.div(new BigNumber(2));
        utility.sign(web3, user, hash, undefined, (err2, sig) => {
          utility.testSend(web3, contractEtherDelta, contractEtherDeltaAddr, 'openMarginAccount', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s, amount, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err3) => {
            assert.equal(err3, undefined);
            utility.testCall(web3, contractEtherDelta, contractEtherDeltaAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s], (err4, result4) => {
              assert.equal(result4.equals(amountGet.minus(amount)), true);
              done();
    ...
    it('Should attempt some margin accounts initiated onchain that should fail', (done) => {
      function testTrade(expiresIn, orderNonce, tokenGet, tokenGive, amountGet, amountGive, amount, account1, account2, accountLevel, callback) {
        let expires = expiresIn;
        web3.eth.getBlockNumber((err, blockNumber) => {
          if (err) callback(err);
          ...
          utility.testSend(web3, contractAccountLevels, contractAccountLevelsAddr, 'setAccountLevel', [account1, accountLevel, { gas: 1000000, value: 0 }], account1, undefined, 0, (err2) => {
            ...
            utility.testCall(web3, contractAccountLevels, contractAccountLevelsAddr, 'accountLevel', [account1], (err3) => {
              assert.equal(err3, undefined);
              utility.testSend(web3, contractEtherDelta, contractEtherDeltaAddr, 'marginOrder', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, { gas: 1000000, value: 0 }], account1, undefined, 0, (err4) => {
                assert.equal(err4, undefined);
                utility.testSend(web3, contractEtherDelta, contractEtherDeltaAddr, 'openMarginAccount', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0', amount, { gas: 1000000, value: 0 }], account2, undefined, 0, (err5) => {
                  assert.equal(!err5, false);
                  callback();
                });
              });
